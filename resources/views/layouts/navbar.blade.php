...
<img class="navbar-brand"   src="{{ asset('public/images/blood.png') }}"   height="40">
                        ...
                        @else
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    {{ Auth::user()->name }} ({{ Auth::user()->blood_group }}) <span class="caret"></span>
                                </a>
                ...
                @else
                    ...
                    <form  class="form-inline my-2 my-lg-0"  method="POST" action="{{ route('updateAvailability') }}/{{ Auth::user()->status }}">
  ...
  <label class="form-check-label" for="defaultCheck1">
    Available / Un-available ?
                              ...
                              <div class="switch6" style="border:0px solid #999999;">
                                <label class="switch">
                                   @if(Auth::user()->status == 0)
                                  <input type="checkbox" checked onclick="if(this.checked){}else{this.form.submit()}">
                                  @else
